...
var supervisorDaemonStartCmd = &cli.Command{
	...
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "server-url",
			Usage: "Orchestrator base URL (e.g. http://127.0.0.1:8088)",
		...
		&cli.StringFlag{
			Name:  "platform",
			Usage: "Manually specify platform (e.g. windows, linux/arm64) to override auto-detection",
			Value: "",
		},
	},
	Action: func(cctx *cli.Context) error {
		...
		if platform == "" {
			platform = build.DefaultPlatform
		}

		return supervisor.StartDaemon(cctx.Context, repoPath, serverUrl, allowedTags, logMaxAge, logRotationTime, logRotationSize, platform)
	},
}
